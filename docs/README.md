...
```sh
$ npm install --global yo generator-leswigul
```
...
Less files are compiled in the `styles` task, which you can customize however you want. E.g. to reduce the usage of `calc()` with [postcss-calc][calc], run:
